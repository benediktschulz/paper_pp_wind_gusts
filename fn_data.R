...
prep_na <- function(df, var_omit = NULL, var_replace = NULL,
                    loc_name = "location", init_name = "init_tm"){
  ###-----------------------------------------------------------------------------
  ###Input
  #df............Data to mutate (data frame)
  #var_omit......Remove NAs of these variables (string or integer vector)
  #..............Default: NULL -> None of the variables
  #var_replace...Replace NAs of these variables with specific mean (string or integer vector)
  #..............Default: NULL -> None of the variables
  #loc_name......Name of location variable in df (string)
  #..............Default: "location" -> COSMO data
  #init_name.....Name of initialization variable in df (string)
  #..............Default: "init_tm" -> COSMO data
  ###-----------------------------------------------------------------------------
  ###Output
  #df............Mutated data frame (data frame)
  ###-----------------------------------------------------------------------------
  ...
  # Remove NA
  if(!is.null(var_omit)){ 
    df <- df[!apply(df[,var_omit], 1, function(x) any(is.na(x))),] }
  ...
  if(!is.null(var_replace)){
    ...
    for(temp_var in na_vars){
      ...
      na_months <- unique(month(df[na_rows, init_name]))
      
      # Get locations including NA
      na_locs <- unique(df[na_rows, loc_name])
      
      # Calculate mean matrix for each month and location
      mean_mtx <- as.matrix(sapply(na_months, function(x_m){
        # Subset if month
        temp_sub <- df[which(month(df[[init_name]]) == x_m),]
        
        # Locationwise mean for month
        sapply(na_locs, function(x_l){
          res <- mean(temp_sub[temp_sub[[loc_name]] == x_l, 
                               temp_var], na.rm = TRUE)
          
          # Special case: Only NA's for month and location
          if(!is.na(res)){ return(res) }
          else{ return( 
            mean(df[which(df[[loc_name]] == x_l), temp_var], 
                 na.rm = TRUE) 
          )}
        })
      }))
      
      # Replace NAs by corresponding mean
      df[na_rows, temp_var] <- sapply(na_rows, function(i) 
        mean_mtx[which(na_locs == df[i, loc_name]),
                 which(na_months == month(df[i, init_name]))] )
    }
  }
...
prep_data <- function(df, loc_path = NULL, pred_ens = c("ens_mean", "ens_sd"), 
                      ...
                      t_sd = NULL, t_spread = NULL, tm_ref = "init_tm",
  ...
  if(any(is.element(c("lat", "lon", "altitude", "orog_DE"), pred_loc))){
  ...
  # Omit ensemble and observation, replace other variables
  df <- prep_na(df = df,
                var_omit = c(pred_ens, "obs"),
                var_replace = pred_add)
  
  # Truncate ensemble standard deviation (if given as predictor)
  if(!is.null(t_sd) & is.element("ens_sd", pred_ens)){
    df[["ens_sd"]] <- pmax(df[["ens_sd"]], t_sd) }
  
  # Truncate ensemble spread (if given as predictor)
  if(!is.null(t_spread) & is.element("ens_spread", pred_ens)){
    df[["ens_spread"]] <- pmax(df[["ens_spread"]], t_spread) }
  
  # Truncate sub-ensemble spread (if given as predictor)
  ...
  if(sort_ens & any(is.element(pred_ens, ens_str))){ 
    df[,ens_str] <- t(apply(df[,ens_str], 1, sort)) }
  
  #### Temporal predictors ####  
  # Cosine transformed day of the year
  if(is.element("yday", pred_tm)){
    df[["yday"]] <- cos(2*pi*(yday(df[[tm_ref]])-1)/
                          yday(str2time(paste0(year(df[[tm_ref]]), "123100")))) }
  ...
  if(is.element("loc_bias", pred_loc)){
    ...
    # Assign bias of location in data
...
assign_spatial_preds <- function(preds, df_train, df_test){
  ...
  ###-----------------------------------------------------------------------------
  ###Output
  #df_test.....Mutated test data frame (data frame)
  ###-----------------------------------------------------------------------------
